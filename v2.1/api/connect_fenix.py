...
def get_campus(user_fenix):
	...
	for i in campus_aux:
		campus_name.append((i['name'].encode("utf-8")))
...
def get_buildings(user_fenix,campus_id):
	...
	if(buildings_aux== None):
		...
		for j in buildings_aux['containedSpaces']:
			if(j['type']=="BUILDING"):
				buildings_name.append((j['name'].encode("utf-8")))
...
def get_floors(user_fenix,building_id):
	...
	if(floors_aux== None):
		...
		for j in floors_aux['containedSpaces']:
			if(j['type']=="FLOOR"):
				floors_name.append((j['name'].encode("utf-8")))
...
def get_rooms(user_fenix,floor_id):
	...
	if(rooms_aux==None):
		...
		for j in rooms_aux['containedSpaces']:
			if(j['type']=="ROOM"):
				#name=j['name']
				#rooms_name.append(name.encode("utf-8"))
				rooms_name.append(j['name'].encode("utf-8"))
